...
Whenever I added or removed a bookmark, the changes wouldn't show up on the screen immediately. Oddly, it worked fine on my local machine (localhost), but once deployed to Vercel, the UI would "freeze" after a submission. I literally had to switch browser tabs and come back to make an update and see my new bookmark.
...
The app now feels completely instant and snappy on Vercel, regardless of network speed, while still maintaining perfect sync with the backend database.
